...
GPIO 4		I2S-DIN
GPIO 5		RTC0INT
GPIO 6		TP-SDA IMU-SDA RTC-SDA SDA
GPIO 7		TP-SCL IMU-SCL RTC-SCL SCL
GPIO 8		IMU-INT1
GPIO 9		IMU-INT2 
GPIO 10		LCD-SCK
GPIO 11		LCD-MOSI
...
GPIO 17		TP-RST
GPIO 18		TP-INT
GPIO 19		SD-SCK SPI-SCK
GPIO 20		SD-CMD SPI-MOSI
GPIO 21		SD-D0  SPI-MISO
GPIO 22		SD-D1  --------
GPIO 23		SD-D2  --------
GPIO 24		SD-D3  SPI-CS
GPIO 25		KEY-BAT
GPIO 26		BAT_EN
GPIO 27		BAT-ADC
GPIO 28		[external]
...
#define ZX_AY_PWM_PIN0 (2)
#define ZX_AY_PWM_PIN1 (3)
#define ZX_BEEP_PIN (4)
...
#define PWR_DELAY 2500 //в милисекундах
...
#include "../lib/bsp/bsp_st7789.h"
#include "../lib/bsp/bsp_lcd_brightness.h"
#include "../lib/bsp/bsp_i2c.h"
#include "../lib/bsp/bsp_cst328.h"
#include "../lib/bsp/bsp_pcf85063.h"
#include "../lib/bsp/bsp_qmi8658.h"
#include "../lib/bsp/bsp_battery.h"
#include "util_touch2joy.h"

uint32_t pwr_timer;
uint16_t press_cntr=0;
struct tm now_tm;
...
static bool softkey_time_pressed 	= false;
static bool softkey_alarm_pressed 	= false;
static bool softkey_A_pressed 		= false;
static bool softkey_B_pressed 		= false;
static bool softkey_Game_A_pressed 	= false;
static bool softkey_Game_B_pressed 	= false;
static bool softkey_reset_pressed 	= false;
...
void set_gw_sys_time(){
	gw_time_t time_set = {0};
	time_set.hours=now_tm.tm_hour;
	time_set.minutes=now_tm.tm_min;
	time_set.seconds=now_tm.tm_sec;
	//printf("%02d:%02d:%02d\n",time_set.hours,time_set.minutes,time_set.seconds);
	gw_system_set_time(time_set);	
}
...
bool pwr_key_state = true;
bool pwr_key_state_last = true;
...
void process_input(){
	...
	if(menu_mode[menu_ptr]==BOOT_LOGO){
		if (bsp_battery_get_key_level() == false){
			//printf("PWR key pressed \n");
	...
	if((menu_mode[menu_ptr]>BOOT_LOGO)&&(menu_mode[menu_ptr]<EMULATION)){
		bsp_cst328_read();
		if(bsp_cst328_get_touch_data(&cst328_data)){
			if(cst328_data.points==1){
				data_joy = decode_touch_menu(cst328_data);
				busy_wait_ms(100);
				joy_pressed = true;
			}
			if(cst328_data.points>1){
				rd_timer=my_millis();
				//busy_wait_ms(100);
				data_joy = decode_touch_all(cst328_data,true);
				joy_pressed = true;
				//printf("[%012d] points:%d x:%d y:%d pressure:%d\r\n", my_millis(), cst328_data.points, cst328_data.coords[0].x, cst328_data.coords[0].y, cst328_data.coords[0].pressure);
			}
			//printf("%d>decode_touch_menu:[%04X]\n",my_millis(),data_joy);
		} 
		if((rd_timer>0)&&((my_millis()-rd_timer)==500)){
			//printf("points:0\n");
			//rd_timer=my_millis();
			data_joy = decode_touch_all(cst328_data,false);
			//printf("%d>decode_touch_menu:[%04X]\n",my_millis(),data_joy);
			joy_pressed = true;
			busy_wait_ms(50);
		}
		if((rd_timer>0)&&((my_millis()-rd_timer)==500)){
			//printf("%d>reset_joy:[%04X]\n",my_millis(),data_joy);
			rd_timer=0;
			data_joy=0;
			joy_pressed = false;
		}
	...
	if(menu_mode[menu_ptr]==EMULATION){
		...
		if (bsp_cst328_get_touch_data(&cst328_data)){
			if(current_hud_mode&HM_SHOW_MAINBTN){
				data_joy = decode_touch_main(cst328_data);
			} else
			if(current_hud_mode&HM_SHOW_GAMEBTN){
				data_joy = decode_touch_game(cst328_data);
			}
			//printf("decode_touch_game:[%04X]\n",data_joy);
			if((data_joy&0xF000)==0xF000){
				if((data_joy&0xFF00)==SOFTKEY_TIME){
					softkey_time_pressed=true;
					softkey_duration = 10;
				}
				if((data_joy&0xFF00)==SOFTKEY_GAME_A){
					softkey_Game_A_pressed=true;
					softkey_duration = 10;
				}
				if((data_joy&0xFF00)==SOFTKEY_GAME_B){
					softkey_Game_B_pressed=true;
					softkey_duration = 10;
				}
				if((data_joy&0xFF00)==SOFTKEY_ALARM){
					softkey_alarm_pressed=true;
					softkey_duration = 10;
				}
				if((data_joy&0xFF00)==SOFTKEY_RESET){
					softkey_reset_pressed=true;
					softkey_duration = 10;
				}
				data_joy=0;
			}
...
void set_def_palette(){
	for(int i=0;i<16;i++){
		...
		bsp_st7789_set_palette(i+PALETTE_SHIFT,RGB);
	}
}

void updatePalette(const uint8_t *palette,uint16_t length) {
	bsp_st7789_set_palette(0, RGB888(0, 0, 0));
    for (uint8_t i = 0; i < (length/3); i++) {
        uint8_t R = palette[0];
        uint8_t G = palette[1];
        uint8_t B = palette[2];
		//printf("Update Pal: [%d]>[%d][%d][%d]\n",i,R,G,B);
        bsp_st7789_set_palette(i, RGB888(R, G, B));
		palette+=3;
    }
...
void updatePaletteGame(const uint8_t *palette,uint16_t length) {
    ...
    for (uint8_t i = 0; i < (length/3); i++) {
		...
		for (uint8_t j = 0; j < 4; j++) {
        	...
        	bsp_st7789_set_palette(i+(PALETTE_IDX_INC*j), RGB888(R_out, G_out, B_out));
		}
		palette+=3;
    }
	//graphics_set_palette(255, 0);
...
uint8_t EditDialogBox(char *header,char *message,char *value,uint8_t colorFG,uint8_t colorBG,uint8_t di_id,bool in_type){
	...
	if(max_len<value_len)max_len=value_len;
	if(max_len<19){max_len=19;}
	volatile uint8_t left_x =(SCREEN_W/2)-((max_len/2)*FONT_W);
	volatile uint8_t left_y = strlen(message)==0 ? (SCREEN_H/2)-(FONT_H/2):(SCREEN_H/2)-FONT_H;
	...
	if (strlen(message)>0){
		draw_text(left_x,left_y,header,CL_WHITE,CL_EMPTY);
		draw_text(left_x,left_y+FONT_H,message,colorFG,CL_EMPTY);
		draw_text_len(left_x,left_y+(FONT_H*2),value,colorFG,CL_WHITE,value_len);
	} else {
...
bool FAST_FUNC(Sound_timer_callback)(repeating_timer_t *rt){
	...
	if((outL!=outL_old)||(outR!=outR_old)){
		...
		*/
		resL = (((int)((2*(mixL+outL))-((mixL*outL)/128)-128)*4)*(cfg_volume/CFG_VOLUME_STEP));
		resR = (((int)((2*(mixR+outR))-((mixR*outR)/128)-128)*4)*(cfg_volume/CFG_VOLUME_STEP));
	}
	i2s_out((int)resL,(int)resR);
...
void get_battery_stats(){
	...
	battery_power_percent = bsp_battery_percent((uint16_t)(voltage*1000));
	//printf("volt:%d   raw:%d  perc:%d \n",((uint16_t)(voltage*1000)),adc_raw,battery_power_percent);
	battery_status = round(battery_power_percent/10)+1;
	if(battery_status>10)battery_status=10;
	if (battery_power_percent>=100){
		battery_status_ico++;
...
bool FAST_FUNC(Emu_callback)(repeating_timer_t *rt){
	if((emulation)){
		...
		//printf("gw_system_run:[%09d]\n",time_us_32());
		if (softkey_duration > 0)	softkey_duration--;
		if (softkey_duration == 1){
			softkey_A_pressed		= false;
			softkey_B_pressed		= false;
			softkey_Game_A_pressed	= false;
			softkey_Game_B_pressed	= false;
			softkey_time_pressed	= false;
			softkey_alarm_pressed	= false;
			printf("btn reset\n");
		}
	}		
	return true;
...
void Render_loop() {
	...
	bsp_st7789_init_render(60000);//585
	while (true) {
		if((emulation)){// &&(!graphics_draw_screen)
			gw_system_blit(&graph_buf[0]);
			//while(graphics_draw_screen){busy_wait_us(5);};
		}
	}
}
...
void graphics_set_hud_handler(bool (*handler)(short)){
	st7789_info.handler=handler;
}

uint16_t color_arr[6] = {0xFFFF, 0x0000, 0x0F1f, 0xf800, 0x07e0, 0x001f};
...
int main(void){
	...
	*/
    ...
    st7789_info.width = SCREEN_W;
    st7789_info.height = SCREEN_H;
	if(cfg_rotate==0) st7789_info.rotation = 1; 
	if(cfg_rotate==1) st7789_info.rotation = 3; 
	...
	st7789_info.pix_format=cfg_pixels;
	printf(" %s %s %s\n",video_out_rotate[cfg_rotate],video_out_inversion[cfg_inversion],video_out_pixels[cfg_pixels]);

    
    cst328_info.width = st7789_info.width;
    cst328_info.height = st7789_info.height;
    cst328_info.rotation = st7789_info.rotation;


   

    st7789_info.data=graph_buf;
	st7789_info.overlay=hud_line;
	...
	//bsp_st7789_set_rotation(cfg_rotate);
	//bsp_st7789_set_inversion(cfg_inversion);
	
	//bsp_st7789_clr_scr(0x7BEF);

    bsp_lcd_brightness_init();
    bsp_lcd_brightness_set(cfg_brightness*10);

	printf("Video Out Started\n");

	updatePalette(mur_logo_pal,LOGO_PAL_SIZE);

	bsp_st7789_clear(0, 0, st7789_info.width-1, st7789_info.height-1, 0x0000);

	/*while(true){
		bsp_st7789_clear(0, 0, st7789_info.width - 1, st7789_info.height - 1,color_arr[color_index]);
	...
	*/
	...
	do{
		...
		printf("\n");
	} while(ptr<INTRO_SOUND_LEN);
	printf("-------------[intro_sound]-------------\n");

	/*for (uint16_t idx; idx<INTRO_SOUND_LEN;idx++){
		uint16_t wav=(uint16_t)((intro_sound[ptr++])&(intro_sound[ptr++]<<8));
	...
	do{ //BEGIN GLOBAL LOOP
		...
		if(menu_mode[menu_ptr]<EMULATION){
			old_hud_mode=current_hud_mode;
			current_hud_mode=0;
			current_hud_mode|=HM_SHOW_MENUBTN;
			current_hud_mode|=HM_SHOW_BATTERY;			
			//if(current_hud_mode&HM_SHOW_MENUBTN){
			handler_ptr=&hud_menu_kb;
			graphics_set_hud_handler(handler_ptr);
			//} else {
			//	graphics_set_hud_handler(NULL);
			//};
				...
				switch (menu_mode[menu_ptr]){
							...
							if(fast_mode[fast_mode_ptr]==FAST_MENU_MAIN){
								switch (fast_menu_index){
									...
									case FAST_ALARM: //Set Alarm
										...
										continue;
										break;
									case FAST_SET_CLOCK: // SET CLOCK
										
										memset(temp_msg,0,sizeof(temp_msg));
										get_sys_time();
										strncpy(temp_msg,&hud_text[1],19);
										//sprintf(temp_msg,"%07d",0);
										uint16_t pos = 0;

										if(EditDialogBox("[SETUP]","Set system time:",temp_msg,CL_BLACK,COLOR_BACKGOUND,DIALOG_OK_CAN,true)==DLG_RES_OK){
											//01.01.2025 12:10:28
											pos = strchr(temp_msg, '.') - temp_msg;
											strncpy(buf, temp_msg, pos);
											now_tm.tm_mday = atoi(buf);          // Day of the month
											memset(buf,0,sizeof(buf));
											strcpy(temp_msg, temp_msg+pos+1);
											pos = strchr(temp_msg, '.') - temp_msg;
											strncpy(buf, temp_msg, pos);
											now_tm.tm_mon = atoi(buf);       // Months start from 0 (November = 10)
											memset(buf,0,sizeof(buf));
											strcpy(temp_msg, temp_msg+pos+1);
											pos = strchr(temp_msg, ' ') - temp_msg;
											strncpy(buf, temp_msg, pos);
											now_tm.tm_year = atoi(buf) - 1900; // The year starts from 1900
											memset(buf,0,sizeof(buf));
											strcpy(temp_msg, temp_msg+pos+1);
											pos = strchr(temp_msg, ':') - temp_msg;
											strncpy(buf, temp_msg, pos);
											now_tm.tm_hour = atoi(buf);          // Hour
											memset(buf,0,sizeof(buf));
											strcpy(temp_msg, temp_msg+pos+1);
											pos = strchr(temp_msg, ':') - temp_msg;
											strncpy(buf, temp_msg, pos);
											now_tm.tm_min = atoi(buf);            // Minute
											memset(buf,0,sizeof(buf));
											strcpy(temp_msg, temp_msg+pos+1);
											now_tm.tm_sec = atoi(temp_msg);            // Second
											now_tm.tm_isdst = -1;         // Automatically detect daylight saving time
											bsp_pcf85063_set_time(&now_tm);
											printf("SET CLOCK: %s \n",temp_msg);
										}
					...
					case MENU_SETTINGS:
						...
						if ((menu_inc_dec!=0)&&(need_redraw==true)){
								...
								if (settings_index==1){ //Soft Sound Vol:
								...
								if (settings_index==2){ //LCD BrightLev:
								...
								if (settings_index==3){ //LCD Rotate:
		...
		if(menu_mode[menu_ptr]==EMULATION){
			memset(hud_line,0x11,SCREEN_W);
			current_hud_mode=0;
			current_hud_mode|=HM_SHOW_MAINBTN;
			current_hud_mode|=HM_SHOW_BATTERY;
			printf("HM1>[%04X]\n",current_hud_mode);
			...
			if(current_hud_mode&HM_SHOW_MAINBTN)	{hud_ptr=&hud_main_kb;};
			if(current_hud_mode&HM_SHOW_GAMEBTN)	{hud_ptr=&hud_game_kb;};
			if(current_hud_mode&HM_SHOW_MENUBTN)	{hud_ptr=&hud_menu_kb;};
			...
			do{
				ticker++;
				busy_wait_us(100);
				if ((ticker%512)==0){ //4608
					process_input();
					if((!joy_pressed)&&(rel_data_joy&D_JOY_START)){
						enter_pause();
						busy_wait_ms(100);
						gw_system_sound_init();
						menu_ptr++;
					...
					if(hat_switch&HAT_SELECT){
						...
						if(cfg_mobile_mode==MOBILE_MURM_ON){
							if(hat_switch&HAT_RIGHT){
								...
								bsp_lcd_brightness_set(cfg_brightness*10);	//уровень подсветки TFT
								hud_timer = my_millis();
				...
				if(((softkey_Game_A_pressed)||(softkey_Game_B_pressed))&&(softkey_duration==5)){
					current_hud_mode=0;
					current_hud_mode|=HM_SHOW_GAMEBTN;
					current_hud_mode|=HM_SHOW_BATTERY;
					printf("start game:[%02X]\n",current_hud_mode);
				}
				...
				if(softkey_reset_pressed){
					...
					current_hud_mode|=HM_SHOW_MAINBTN;
					current_hud_mode|=HM_SHOW_BATTERY;
					softkey_reset_pressed=false;
					softkey_time_pressed=true;
					softkey_duration = GW_REFRESH_RATE;
				}

				if ((bsp_battery_get_key_level() == false)&&(pwr_timer==0)){
					//printf("pwr press\n");
					pwr_timer=my_millis();
				}
				if ((bsp_battery_get_key_level() == false)&&(pwr_timer>0)&&((ticker%1024)==0)){
					press_cntr++;
					//printf("pwr press: %d\n",press_cntr);
				}
				if ((bsp_battery_get_key_level() == false)&&((my_millis()-pwr_timer)==(PWR_DELAY))&&(press_cntr>20)){
					//printf("pwr OFF\n");
					emulation=false;
					busy_wait_ms(100);
					bsp_battery_enabled(false);
					menu_ptr=0;
					menu_mode[menu_ptr]=BOOT_LOGO;
					break;
				}
				if((my_millis()-pwr_timer)>(PWR_DELAY+500)){
					pwr_timer=0;
					press_cntr=0;
				}
