...
void bsp_i2s_init(void)
{
    ...
    dma_channel_configure(dmaSoundTx, &c, (uint32_t *)&(BSP_I2S_SOUND_PIO->txf[i2sSoundSm]), &i2s_data, 1<<10, false);
	//dma_start_channel_mask((1u << dmaSoundTx)) ;
}
...
inline void bsp_i2s_out(int16_t l_out,int16_t r_out){i2s_data=(((uint16_t)l_out)<<16)|(((uint16_t)r_out));};
